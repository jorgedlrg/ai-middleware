...
@Transactional
@Slf4j
public class AutoreplyUseCaseImpl implements AutoreplyUseCase {
  ...
  public void execute(Command cmd) {
    if (cmd.autoreplyRole().isPresent()) {
      UUID autoreplyRole = cmd.autoreplyRole().get();

      Session session = getSessionByIdOutPort.query(cmd.session()).orElseThrow();
      if (!session.getLastInteraction().getRole().equals(autoreplyRole)) {
        ...
        Actor actingActor =
            getActorByIdOutPort.query(session.getFeaturedActor(autoreplyRole).get()).orElseThrow();
        ...
        log.info(
            String.format(
                "Last interaction belongs to selected autoreply role %s. Not replying.",
                autoreplyRole));
      }
    }
  }
